...
default_train_transforms_KITTI = [
...
default_train_transforms_NUSCENES = [
    Resize3D((375,1242)),
    # PhotometricDistortion(
    #     brightness_delta=32,
    #     contrast_range=(0.5, 1.5),
...
default_test_transforms_KITTI = [
    Normalize(mean=[123.675, 116.28, 103.53], std=[58.395, 57.12, 57.375]),
    Pad(size_divisor=32),
    ToTensor(),
]

default_test_transforms_NUSCENES = [
    Resize3D((375,1242)),
    # Normalize(mean=[123.675, 116.28, 103.53], std=[58.395, 57.12, 57.375]),
    Pad(size_divisor=32),
    ToTensor(),
...
class MonoDataset(BaseKITTIMono3DDataset):
    def __init__(self, 
        ...
        if transforms is None:
            if (split == 'train'):
                if not nuscenes:
                    transforms = default_train_transforms_KITTI
                else:
                    transforms = default_train_transforms_NUSCENES
            else:
                if not nuscenes:
                    transforms = default_test_transforms_KITTI
                else:
                    transforms = default_test_transforms_NUSCENES
    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        ...
        raw_labels = self.load_label(idx,self.nuscenes)
        if not self.nuscenes:
