...
class BaseKITTIMono3DDataset(Dataset):
    def __init__(self, 
        ...
        with open(os.path.join(base_root,"ImageSets", f'{split}.txt')) as f:
        ...
        self.gt_annos = None
        if preload_gt_info:
            gt_infos = self.collect_gt_infos(nuscenes=self.nuscenes)
    ...
    def load_label(self, idx: int,nuscenes) -> KITTIMultiObjects:
        calib = self.load_calib(idx)
        return KITTIMultiObjects.get_objects_from_label(self.label_files[idx], calib,nuscenes)
    ...
    def collect_gt_infos(self, verbose: bool = False) -> List[Dict[str, Any]]:
        ...
        ignored_flag = False
        if self.load_label(0,self.nuscenes).ignore_dontcare:
            ignored_flag = True
        ...
        for idx in iter_:
            ...
            obj_cls = self.load_label(idx,self.nuscenes)
            if ignored_flag:
                obj_cls = obj_cls.original_objects
            if (self.nuscenes and  len(obj_cls) == 0):
                obj_dict = {}    
            else: obj_dict = obj_cls.info_dict
